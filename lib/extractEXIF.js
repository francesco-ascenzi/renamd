 ...
 * @license Apache 2.0
 */
export default async function extractEXIF(filePath) {
    ...
    try {
        ...
        let searchNextChunk = -1; // searchNextChunk == 0: full, 1: without 0xE1, 2: without EXIF length marker
        ...
        for await (const chunk of readStream) {
            ...
            if (searchNextChunk != -1) {
                if (searchNextChunk == 0) {
                    if (chunk[0] == 0xE1) {
                        // Check EXIF length
                        let lengthMarker = chunk.readUInt16BE(1);
                    ...
                    // Check EXIF length
                    let lengthMarker = chunk.readUInt16BE(0);
                    // Retrieve remaining data from the next chunk
            ...
            while (startIndex != -1) {
                ...
                if (chunk[startIndex + 1] > chunkLength) {
                    ...
                    if (chunk[startIndex + 2] > chunkLength) {
                        ...
                        // Check EXIF length
                        let lengthMarker = chunk.readUInt16BE(startIndex + 2);
                        // Retrieve remaining data from the next chunk
    ...
    // Format Exif
    const exifData = exif(bufferChunk);
